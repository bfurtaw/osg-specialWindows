...
class Document
{
    public:
        ...
        void pushExtendedMaterial(Record *hdr ) {
        	firstPush.push_back(hdr);
        	OSG_WARN << "Value of ExtendedMaterial " << hdr << " size_of stack " << firstPush.size() << std::endl;
        }
        Record * popExtendedMaterial() {
        	Record *hdr;
        	//OSG_WARN << "Size of the Stack " << firstPush.size() << std::endl;
        	if (!firstPush.empty()) {
        		hdr = firstPush.back(); assert (hdr);  /* LINUX */
        		firstPush.pop_back();
        		return hdr;
        	} else {
        		return NULL;
        ...
        Record * getCurrentExtendedMaterial() {
        	if( firstPush.size() > 0)
        		return firstPush.back();
        	else OSG_FATAL << "No extendedMaterialRecords: Invalid OpenFlight file" << std::endl;
        	return NULL;
        }
        ...
        void setExtendedMaterialPool(ExtendedMaterialPool* cp, bool parent=false) { _extendedMaterialPool = cp; _extendedMaterialPoolParent=parent; }
