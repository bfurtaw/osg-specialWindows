...
protected:
    ...
    osg::StateSet* readTexture(const std::string& filename, const Document& document, int32 index) const
    {
        ...
        texture->setImage(image.get());
		  //texture->UniqueID(index);	// XXX blf: set the Creator ID here so TextureUsageType match can be found later.
    ...
    virtual void readRecord(RecordInputStream& in, Document& document)
    {
        ...
        OSG_WARN << "Loaded model texture(" << index << ") " << filename;
        ...
        if (!stateset)
        {
            stateset = readTexture(pathname,document, index);
            OSG_WARN << " NOT in Texture Cache." << std::endl;
